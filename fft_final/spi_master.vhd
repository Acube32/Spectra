...
--              Sends a Data one bit at a time on MOSI
--              Will also receive Data data one bit at a time on MISO.
--              Any data on input Data will be shipped out on MOSI.
--
--              To kick-off transaction, user must pulse i_TX_DV.
--              This module supports multi-Data transmissions by pulsing
--              i_TX_DV and loading up i_TX_Data when o_TX_Ready is high.
--
...
entity spi_master is
  ...
  port (
   ...
   -- TX (MOSI) Signals
   i_TX_Data   : in std_logic_vector(15 downto 0);   -- Data to transmit on MOSI
   i_TX_DV     : in std_logic;          -- Data Valid Pulse with i_TX_Data
   o_TX_Ready  : out std_logic;         -- Transmit Ready for next Data
   
   -- RX (MISO) Signals
   o_RX_DV   : out std_logic;    -- Data Valid pulse (1 clock cycle)
   o_RX_Data : out std_logic_vector(15 downto 0);   -- Data received on MISO
...
architecture RTL of spi_master is
  ...
  signal r_RX_Bit_Count : unsigned(3 downto 0);
...
begin
  ...
  begin
    if i_Rst_L = '0' then
      ...
      if i_TX_DV = '1' then
        o_TX_Ready      <= '0';
        r_TX_Ready      <= '0';
        r_SPI_Clk_Edges <= 32;  -- Total # edges in one Data ALWAYS 32
  ...
  -- Purpose: Register i_TX_Data when Data Valid is pulsed.
  -- Keeps local storage of Data in case higher level module changes the data
  Data_Reg : process (i_Clk, i_Rst_L)
  begin
    if i_Rst_L = '0' then
      r_TX_Data <= X"0000";
      r_TX_DV   <= '0';
  ...
  begin
    if i_Rst_L = '0' then
      ...
      -- Catch the case where we start transaction and CPHA = 0
        ...
        o_SPI_MOSI     <= r_TX_Data(15);
        r_TX_Bit_Count <= "1110";        -- 14
      elsif (r_Leading_Edge = '1' and w_CPHA = '1') or (r_Trailing_Edge = '1' and w_CPHA = '0') then
        r_TX_Bit_Count <= r_TX_Bit_Count - 1;
        o_SPI_MOSI     <= r_TX_Data(to_integer(r_TX_Bit_Count));
      end if;
    end if;
  ...
  begin
    if i_Rst_L = '0' then
      o_RX_Data      <= X"0000";
      o_RX_DV        <= '0';
      r_RX_Bit_Count <= "1111";          -- Starts at 15
    elsif rising_edge(i_Clk) then
      -- Default Assignments
      o_RX_DV <= '0';
      ...
      if r_TX_Ready = '1' then -- Check if ready, if so reset count to default
        ...
        o_RX_Data(to_integer(r_RX_Bit_Count)) <= i_SPI_MISO;  -- Sample data
        r_RX_Bit_Count <= r_RX_Bit_Count - 1;
        if r_RX_Bit_Count = "0000" then
          o_RX_DV <= '1';   -- Data done, pulse Data Valid
        end if;
      end if;
    end if;
